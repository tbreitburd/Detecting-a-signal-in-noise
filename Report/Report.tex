...
This can be done within a Docker container, by building an image from the \texttt{Dockerfile}, and once in a running container, switching to the created environment, and the project's directory (\texttt{S1\_Coursework}):

\vspace*{1\baselineskip}
\begin{lstlisting}[language=bash, caption={Changing to the correct directory and environment, in the Docker container}]
    $ conda activate PrincDSCW
    $ cd S1_Coursework
\end{lstlisting}
